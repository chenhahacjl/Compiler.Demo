...
1. **A4-1 ✅（range-for 降序 step）**：step 校验由"常量正整数"放宽为"常量非零整数"（正=升序、负=降序；零=死循环仍拒，诊断更新）。`Lowerer.RewriteForStatement` 方向感知：负 step → `i >= upper` 继续、`i = i + step`（负即递减）；新增 `BoundNodeFactory.GreaterOrEqual`。native `BoundTreeToIr` ForStatement 防御分支同步方向感知（主路径已在绑定期经通用 Lowerer 降级，该分支现确认死代码）。三后端回归：Evaluator 4 例（降序/步长 2/永不命中/升序步长 2）+ IL e2e + native x64/x86 e2e。全量 41724 通过 / 2 skip / 1 环境锁（total 41727）。