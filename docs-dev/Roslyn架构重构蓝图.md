...
2. **A4-2 ✅（`to` 自动定方向——无 downto 关键字）**：按用户要求不新增 `downto`，`for i = 10 to 1` 直接降序。语义：两界为编译期常量且 `lower > upper` → 自动降序（缺省 step -1）；否则方向以显式 step 符号为准（A4-1 保留）；step 一律按**幅值**（`10 to 1 step 2` 亦降序 2）。实现全在 binder（`BindForStatement`：常量折叠判方向 + 幅值→带符号 step），Lowerer/native 沿用 A4-1 负 step 机制零改动。既有 `for (var i = 10 to 1)` 空循环断言（0）更新为自动降序（10 次）；step 诊断消息简化为"常量非零整数"。回归：Evaluator 2 例 + IL/native e2e 各补 2 项；全量 41726 通过 / 2 skip / 1 环境锁（total 41729）。