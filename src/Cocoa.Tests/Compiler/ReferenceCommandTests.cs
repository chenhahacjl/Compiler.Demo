...
namespace Cocoa.Tests.Compiler
{
    public class ReferenceCommandTests
    {
        ...
        public void AddReference_CreatesReferencesSection()
        {
            ...
            var (exitCode, stdout, stderr) = CliTestRunner.Run("add reference -p App.coproj ../Libs/MyLib.cod", dir);
        ...
        public void AddReference_Duplicate_IsIdempotent()
        {
            ...
            var first = CliTestRunner.Run("add reference -p App.coproj ../Libs/MyLib.cod", dir);
            Assert.True(first.ExitCode == 0, first.Stderr);

            var second = CliTestRunner.Run("add reference -p App.coproj ../Libs/MyLib.cod", dir);
        ...
        public void RemoveReference_RemovesLine()
        {
            ...
            var (exitCode, stdout, stderr) = CliTestRunner.Run("remove reference -p App.coproj ../Libs/MyLib.cod", dir);

            Assert.True(exitCode == 0, stderr);
            Assert.Contains("Removed reference", stdout);
            Assert.DoesNotContain("../Libs/MyLib.cod", File.ReadAllText(Path.Combine(dir, "App.coproj")));
        }
