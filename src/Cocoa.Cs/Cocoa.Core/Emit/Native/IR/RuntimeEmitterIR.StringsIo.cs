...
namespace Cocoa.CodeAnalysis.Emit.Native.IR
{
    ...
    internal static partial class RuntimeEmitterIR
    {
        private sealed partial class RuntimeFunctionEmitter
        {
            ...
            // 字符串对象 = 堆指针 [len:4][chars:2×len]（无 null 结尾），Win32 宽字符串 API 需 LPCWSTR → WidePtrZ 复制补 null。
            // SysCall 上限 6 参：文件 IO 走 ucrtbase 低参 API + 6 参 MultiByteToWideChar；WideCharToMultiByte(8 参) 用手动编码替代。
            // ------------------------------------------------------------------

            /// <summary>取 CO 字符串对象的宽字符区指针（chars@4）。</summary>
            private IrVirtualRegister WidePtr(IrVirtualRegister s)
            {
                var p = NewReg(8);
                Lea(p, s, 4);
                return p;
            ...
            /// 单 helper 内立即消费；两个参数（src/dst）须用不同缓冲，否则互相覆盖。
            /// </summary>
            private IrVirtualRegister WidePtrZInto(IrVirtualRegister s, string bufferKey)
            {
                var path = WidePtr(s);
            ...
            // GetEnvironmentVariable(name:8) → string：GetEnvironmentVariableW 两阶段；未命中 → 空串（对齐 Evaluator ?? ""）
            ...
            // GetCurrentDirectory() → string：GetCurrentDirectoryW 两阶段；空 → 空串
            ...
            private void EmitGetExecutablePath()
            {
                ...
                Jcc(IrCond.Equal, empty);
                Cmp(actual, cap);
                Jcc(IrCond.GreaterOrEqual, empty);
                var lenBytes = NewReg(4);
                Imul(lenBytes, actual, C(4, 2));
                var result = NewReg(8);
                CallRuntime(result, "AllocStringFromBuf", buf, lenBytes);
            ...
            // → MultiByteToWideChar(CP_UTF8) 两阶段直写串对象 chars 区。读缓冲动态 Alloc（32KB），失败 → 空串。
            ...
            private void EmitFileWriteAllText()
            {
                ...
                Mark(loop1);
                Cmp(q, end);
                Jcc(IrCond.GreaterOrEqual, countDone);
                var c = NewReg(4);
                Load(c, q, 0, 2);
                Cmp(c, 0x80);
                Jcc(IrCond.AboveOrEqual, two);
                AddI(count, count, 1);
                AddI(q, q, 2);
                Jmp(loop1);
                Mark(two);
                Cmp(c, 0x800);
                Jcc(IrCond.AboveOrEqual, three);
                AddI(count, count, 2);
                AddI(q, q, 2);
                Jmp(loop1);
                Mark(three);
                Cmp(c, 0xD800);
                Jcc(IrCond.Less, threeByte);
                Cmp(c, 0xDC00);
                Jcc(IrCond.AboveOrEqual, threeByte);
                AddI(count, count, 4);
                AddI(q, q, 4);
                Jmp(loop1);
                Mark(threeByte);
                ...
                Mark(loop2);
                Cmp(q2, end);
                Jcc(IrCond.GreaterOrEqual, encodeDone);
                var ch = NewReg(4);
                Load(ch, q2, 0, 2);
                Cmp(ch, 0x80);
                Jcc(IrCond.AboveOrEqual, t2);
                Store(outp, 0, ch, 1);
                AddI(outp, outp, 1);
                AddI(q2, q2, 2);
                Jmp(loop2);
                ...
                Mark(t3);
                Cmp(ch, 0xD800);
                Jcc(IrCond.Less, t3enc);
                Cmp(ch, 0xDC00);
                Jcc(IrCond.AboveOrEqual, t3enc);
                Jmp(t4);
                Mark(t3enc);
                var e1 = NewReg(4);
                Shr(e1, ch, 12);
                ...
                Mark(encodeDone);
                // 复制路径到 _fileBuffer 并补 null（CO 串无 null 结尾；32KB 缓冲足够，超长路径截断属文档限制）
                var pb = WidePtrZ(_args[0]);
                var wb = NewReg(8);
                LeaData(wb, _wbMode);
                var fp = NewReg(8);
