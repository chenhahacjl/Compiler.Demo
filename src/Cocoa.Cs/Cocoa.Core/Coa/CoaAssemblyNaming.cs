namespace Cocoa.CodeAnalysis.Coa
{
    ...
    internal static class CoaAssemblyNaming
    {
        ...
        public static string ManagedAssemblyName(string coaBaseName)
        {
            return coaBaseName + ManagedSuffix;
        }

        /// <summary>native 库文件基名（阶段 B 预留）：X → X.Native.x64 / X.Native.x86。</summary>
        public static string NativeAssemblyName(string coaBaseName, string architecture)
        {
            return coaBaseName + ".Native." + architecture;
        }
    }
}
