...
namespace Cocoa.CodeAnalysis.CSharp.Binding
{
    ...
    public partial class CSharpBinder
    {
        ...
        private NamedTypeSymbol DeclareClassGroup(List<(ClassDeclarationSyntax Syntax, string Namespace)> parts)
        {
            ...
            if (primary.Syntax.Modifiers.Any(m => m.Kind == CoreSyntax.SyntaxKind.FacadeKeyword) &&
                !isStructDecl &&
            ...
            if (isStruct)
            {
                var isFacadeStruct = parts.Any(p => p.Syntax.Modifiers.Any(m => m.Kind == CoreSyntax.SyntaxKind.FacadeKeyword));
        ...
        private BoundStatement? TryBindEventSubscription(AssignmentExpressionSyntax syntax)
        {
            ...
            if (syntax.Target.Kind == CoreSyntax.CSharpSyntaxKind.MemberAccessExpression)
            {
            ...
            else if (syntax.Target.Kind == CoreSyntax.CSharpSyntaxKind.NameExpression && _currentClass != null)
            {
        ...
        private static ImmutableArray<BoundStatement> BindFieldInitializerStatements(CSharpBinder binder, NamedTypeSymbol classType, bool isStatic)
        {
        ...
        private static BoundStatement BindFieldInitializer(CSharpBinder binder, FieldSymbol field, ExpressionSyntax initializerSyntax)
        {
