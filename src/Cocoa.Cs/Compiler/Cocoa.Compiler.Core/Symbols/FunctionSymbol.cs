...
namespace Cocoa.CodeAnalysis.Symbols
{
    public sealed class FunctionSymbol : Symbol
    {
        ...
        public PropertySymbol? ContainingProperty { get; set; }
        ...
        /// </summary>
        public System.Collections.Generic.List<VariableSymbol>? CapturedVariables { get; set; }

        /// <summary>提升 lambda 专有：其环境对象所属的宿主函数（嵌套 lambda 继承最外层非 lambda 函数）。</summary>
        public FunctionSymbol? EnvironmentOwner { get; set; }

        /// <summary>提升 lambda 专有：调用时须把 Receiver（环境对象）压入环境栈。</summary>
        public bool IsLambdaWithEnvironment { get; set; }
