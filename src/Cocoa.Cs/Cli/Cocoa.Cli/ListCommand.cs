    ...
    internal static class ListCommand
    {
        ...
        private static int ListProjects(string? path)
        {
            ...
            if (entry == null)
            {
                var files = Directory.GetFiles(Environment.CurrentDirectory, "*.cosln").Length > 0
                    ...
                    : Directory.GetFiles(Environment.CurrentDirectory, "*.coproj")
                               .Concat(Directory.GetFiles(Environment.CurrentDirectory, "*.coproj")).ToArray();
            ...
            if (Directory.Exists(entry))
            {
                ...
                var projects = Directory.GetFiles(entry, "*.coproj").ToArray();
        ...
        private static string? ResolveEntry(string? path, bool requireSingle, out bool isDirectory)
        {
            ...
            if (Directory.Exists(path))
            {
                ...
                var projects = Directory.GetFiles(path, "*.coproj").ToArray();
            ...
            if (!path.EndsWith(".coproj", StringComparison.OrdinalIgnoreCase) &&
                !path.EndsWith(".coproj", StringComparison.OrdinalIgnoreCase) &&
