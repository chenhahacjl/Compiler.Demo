...
> 最新：**6e-M23 §6k out/ref 参数完整版（2026-08-26）已落地 ×三后端：`out`/`ref` 修饰符 + 普通参数可赋值 + 明确赋值分析（CFG 前向数据流，三诊断对齐 C#）+ 签名身份五处加修饰符位 + `.cod` par 行扩展；stdlib 新增 `Int32.TryParse(s, out v)`（i64 累加防溢出，±边界全对）；连带修复 native 实参求值顺序右→左 → 左→右（对齐 C#/IL/Evaluator）；全量 **37020** 测试通过**；详见 [`docs-dev/out与ref参数设计.md`](docs-dev/out与ref参数设计.md) 与 [`docs-dev/开发计划.md`](docs-dev/开发计划.md) §6k**；自举前置盘点见 [`docs-dev/自举缺口分析.md`](docs-dev/自举缺口分析.md)**；6e-M22 §6j C5+ 事件多播完整版（2026-08-25）已落地：函数值数组存储 + `+=` 尾插 / `-=` 引用相等移除首匹配 / 类内触发判空快照遍历 + 外部禁读禁调(CS0070) ×三后端**；6e-M22 §6j 委托/Lambda/闭包/event/delegate（2026-08-24）已落地：函数类型 `(A,B)->R` + Func/Action/Predicate 家族 + Lambda 提升/方法组转换/闭包捕获 ×三后端 + delegate 声明(sealed class extends MulticastDelegate) + List<T> 泛型动态数组；详见 [`docs-dev/委托与Lambda设计.md`](docs-dev/委托与Lambda设计.md) 与 [`docs-dev/开发计划.md`](docs-dev/开发计划.md) §6j**；6e-M20 泛型 G0-G6（2026-08-24）已收官：`<T>` 类型参数/实参、where 约束子句、泛型方法调用、Monomorphizer 单态化、List<T> 实例化类型与 `Swap<int>(…)` 显式实参调用全解析（嵌套泛型 `>>` 拆分），绑定层单态化（G1/G2）进行中**；6h 对象模型 M5（2026-08-24）已收官：null 字面量 / is-as / any== 收尾——System.Object 基类成员面、用户类 override 虚分派（IL 槽复用 / REPL 运行时链分派 / native vtable）、System.Type 三后端一致；6e-M13（2026-08-20）：`coc` / `coi` 合并为单一 `cocoa` 命令（`-i` 进入 REPL）+ dotnet 式子命令（`new` / `list` / `add reference` / `remove reference` / `run` / `clean`）；核心库更名 `Cocoa.Core`，缓存目录 `.cocoa/`（详见 [`docs/编译手册.md`](docs/编译手册.md) §3）；6e-M15 双前端拆分：`.co` 宽松主方言 / `.cs` 严格 C# 方言（详见 [`docs/语法手册.md`](docs/语法手册.md) §46）
