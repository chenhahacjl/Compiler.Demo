...
> 最新：**G7-core 泛型 .coa 序列化消费闭环（2026-08-26）已落地 ×三后端：gcls/tpar/fld 节点 + 开放绑定体随库携带 + BoundTreeSubstituter 替换展开 + 用户泛型库消费 e2e 全对；6e-M23 §6k out/ref 参数完整版（2026-08-26）：out/ref 修饰符 + 普通参数可赋值 + 明确赋值分析（对齐 C#）+ Int32.TryParse(s, out v)**；详见 [docs-dev/开发计划.md](docs-dev/开发计划.md) §6i G7 行/§6k**
